import { Q as L, a as A, j as l, V as D, f as ce, W as de, D as B, d as C, e as T, B as he, X as k, Y as w, w as v, Z as y, q as pe, _ as le, $ as z, M as F, a0 as O, a1 as fe, a2 as I, a3 as ge, S as W, T as M, A as me, R as _e, r as S } from "./wmqEO8lS.js";
import { c as be, u as ye, U as xe, B as Ge, G as Pe, e as Be, l as Se, a as Te, t as Ce } from "./DhvZDYTi.js";
import { R as ve, S as Ue, d as we } from "./Cl2A_dXD.js";
import { C as R } from "./DyqTcbkf.js";
const G = A.for2d();
class H {
  start(e, t, r) {
  ...
  execute(e, t) {
    ...
    const n = s.bindGroup.getBindGroup(t.bindGroup, r, 1), o = s.pipeline.getPipeline(this._geometry, r, G, t.topology);
...
H.extension = { type: [l.WebGPUPipesAdaptor], name: "batch" };
class V {
  constructor(e) {
  ...
  _createBindGroup(e, t, r) {
    ...
    for (const g in i) {
      const h = e.resources[g] ?? e.resources[i[g]];
      let p;
      if (h._resourceType === "uniformGroup") {
        const d = h;
        o.ubo.updateUniformGroup(d);
        const f = d.buffer;
        p = { buffer: o.buffer.getGPUBuffer(f), offset: 0, size: f.descriptor.size };
      } else if (h._resourceType === "buffer") {
        const d = h;
        p = { buffer: o.buffer.getGPUBuffer(d), offset: 0, size: d.descriptor.size };
      } else if (h._resourceType === "bufferResource") {
        const d = h;
        p = { buffer: o.buffer.getGPUBuffer(d.buffer), offset: d.offset, size: d.size };
      ...
      n.push({ binding: i[g], resource: p });
    }
    const u = o.shader.getProgramData(t).bindGroups[r], c = s.createBindGroup({ layout: u, entries: n });
...
V.extension = { type: [l.WebGPUSystem], name: "bindGroup" };
class Me {
  constructor(e) {
    this.gpuBuffer = e;
  }
...
class N {
  constructor(e) {
    this._renderer = e, this._managedBuffers = new D({ renderer: e, type: "resource", onUnload: this.onBufferUnload.bind(this), name: "gpuBuffer" });
  }
  contextChange(e) {
    this._gpu = e;
  }
  getGPUBuffer(e) {
    var _a;
    return e._gcLastUsed = this._renderer.gc.now, ((_a = e._gpuData[this._renderer.uid]) == null ? void 0 : _a.gpuBuffer) || this.createGPUBuffer(e);
  }
  updateBuffer(e) {
...
N.extension = { type: [l.WebGPUSystem], name: "buffer" };
class Re {
...
j.extension = { type: [l.WebGPUSystem], name: "colorMask" };
class U {
  constructor(e) {
    this._renderer = e;
  }
  ...
  async _createDeviceAndAdaptor(e) {
    const t = await B.get().getNavigator().gpu.requestAdapter({ powerPreference: e.powerPreference, forceFallbackAdapter: e.forceFallbackAdapter }), r = ["texture-compression-bc", "texture-compression-astc", "texture-compression-etc2"].filter((i) => t.features.has(i)), s = await t.requestDevice({ requiredFeatures: r });
...
U.extension = { type: [l.WebGPUSystem], name: "device" };
U.defaultOptions = { powerPreference: void 0, forceFallbackAdapter: false };
class K {
  constructor(e) {
  ...
  restoreRenderPass() {
    const e = this._renderer.renderTarget.adaptor.getDescriptor(this._renderer.renderTarget.renderTarget, false, [0, 0, 0, 1], this._renderer.renderTarget.mipLevel, this._renderer.renderTarget.layer);
...
K.extension = { type: [l.WebGPUSystem], name: "encoder", priority: 1 };
class q {
  constructor(e) {
    this._renderer = e;
  }
...
q.extension = { type: [l.WebGPUSystem], name: "limits" };
class Y {
  constructor(e) {
  ...
  onRenderTargetChange(e) {
    ...
    t || (t = this._renderTargetStencilState[e.uid] = { stencilMode: C.DISABLED, stencilReference: 0 }), this._activeRenderTarget = e, this.setStencilMode(t.stencilMode, t.stencilReference);
  }
  setStencilMode(e, t) {
...
function Ae(a) {
  return be(a, "uboWgsl", Le, ye);
}
class $ extends xe {
  constructor() {
    super({ createUboElements: Ee, generateUboSync: Ae });
  }
}
$.extension = { type: [l.WebGPUSystem], name: "ubo" };
const b = 128;
class X {
  constructor(e) {
    ...
    for (let r = 0; r < t; r++) {
      ...
      r === 0 && (s |= T.COPY_SRC), this._buffers.push(new he({ data: this._batchBuffer.data, usage: s }));
    }
  }
  renderEnd() {
  ...
  _getBufferResource(e) {
    if (!this._bufferResources[e]) {
      ...
      this._bufferResources[e] = new Ge({ buffer: t, offset: (e / 2 | 0) * 256, size: b });
    }
    return this._bufferResources[e];
  }
  _getBindGroup(e) {
    if (!this._bindGroups[e]) {
      const t = new k({ 0: this._getBufferResource(e) });
...
X.extension = { type: [l.WebGPUPipes], name: "uniformBatch" };
const De = { "point-list": 0, "line-list": 1, "line-strip": 2, "triangle-list": 3, "triangle-strip": 4 };
...
class Z {
  ...
  _createPipeline(e, t, r, s) {
    ...
    const c = this._renderer.shader.getProgramData(t).pipeline, g = { vertex: { module: this._getModule(t.vertex.source), entryPoint: t.vertex.entryPoint, buffers: n }, fragment: { module: this._getModule(t.fragment.source), entryPoint: t.fragment.entryPoint, targets: o }, primitive: { topology: s, cullMode: r.cullMode }, layout: c, multisample: { count: this._multisampleCount }, label: "PIXI Pipeline" };
    return this._depthStencilAttachment && (g.depthStencil = { ...this._stencilState, format: "depth24plus-stencil8", depthWriteEnabled: r.depthTest, depthCompare: r.depthTest ? "less" : "always" }), i.createRenderPipeline(g);
  }
  _getModule(e) {
  ...
  getBufferNamesToBind(e, t) {
    ...
    for (let o = 0; o < s.length; o++) {
      ...
      for (const g in n) if (n[g].location === c) {
        i[o] = g;
        break;
      }
    }
  ...
  _createVertexBufferLayouts(e, t) {
    ...
    return e.buffers.forEach((i) => {
      const n = { arrayStride: 0, stepMode: "vertex", attributes: [] }, o = n.attributes;
      for (const u in t.attributeData) {
        const c = e.attributes[u];
        (c.divisor ?? 1) !== 1 && v(`Attribute ${u} has an invalid divisor value of '${c.divisor}'. WebGPU only supports a divisor value of 1`), c.buffer === i && (n.arrayStride = c.stride, n.stepMode = c.instance ? "instance" : "vertex", o.push({ shaderLocation: t.attributeData[u].location, offset: c.offset, format: c.format }));
      }
      o.length && s.push(n);
...
Z.extension = { type: [l.WebGPUSystem], name: "pipeline" };
class Fe {
  constructor() {
...
class Oe {
  ...
  copyToTexture(e, t, r, s, i) {
    ...
    return n.encoder.commandEncoder.copyTextureToTexture({ texture: o, origin: r }, { texture: u, origin: i }, s), t;
  }
  startRenderPass(e, t = true, r, s, i = 0, n = 0) {
    var _a, _b;
    const u = this._renderTargetSystem.getGpuRenderTarget(e);
    if (n !== 0 && ((_a = u.msaaTextures) == null ? void 0 : _a.length)) throw new Error("[RenderTargetSystem] Rendering to array layers is not supported with MSAA render targets.");
    if (i > 0 && ((_b = u.msaaTextures) == null ? void 0 : _b.length)) throw new Error("[RenderTargetSystem] Rendering to mip levels is not supported with MSAA render targets.");
  ...
  getDescriptor(e, t, r, s = 0, i = 0) {
    ...
    const n = this._renderTargetSystem, o = n.getGpuRenderTarget(e), u = e.colorTextures.map((h, p) => {
      ...
      } else f = this._renderer.texture.getGpuSource(h).createView({ dimension: "2d", baseMipLevel: s, mipLevelCount: 1, baseArrayLayer: i, arrayLayerCount: 1 });
      o.msaaTextures[p] && (m = f, f = this._renderer.texture.getTextureView(o.msaaTextures[p]));
      const x = t & y.COLOR ? "clear" : "load";
      return r ?? (r = n.defaultClearColor), { view: f, resolveTarget: m, clearValue: r, storeOp: "store", loadOp: x };
    });
    let c;
    if ((e.stencil || e.depth) && !e.depthStencilTexture && (e.ensureDepthStencilTexture(), e.depthStencilTexture.source.sampleCount = o.msaa ? 4 : 1), e.depthStencilTexture) {
      ...
      c = { view: this._renderer.texture.getGpuSource(e.depthStencilTexture.source).createView({ dimension: "2d", baseMipLevel: s, mipLevelCount: 1, baseArrayLayer: i, arrayLayerCount: 1 }), stencilStoreOp: "store", stencilLoadOp: h, depthClearValue: 1, depthLoadOp: p, depthStoreOp: "store" };
    }
    return { colorAttachments: u, depthStencilAttachment: c };
  }
  clear(e, t = true, r, s, i = 0, n = 0) {
    if (!t) return;
  ...
  initGpuRenderTarget(e) {
    ...
    return t.colorTargetCount = e.colorTextures.length, e.colorTextures.forEach((r, s) => {
      if (r instanceof pe) {
        ...
        try {
          i.configure({ device: this._renderer.gpu.device, usage: GPUTextureUsage.TEXTURE_BINDING | GPUTextureUsage.COPY_DST | GPUTextureUsage.RENDER_ATTACHMENT | GPUTextureUsage.COPY_SRC, format: "bgra8unorm", alphaMode: n });
        } catch (o) {
          console.error(o);
        }
        t.contexts[s] = i;
      ...
      if (t.msaa = r.source.antialias, r.source.antialias) {
        const i = new le({ width: 0, height: 0, sampleCount: 4, arrayLayerCount: r.source.arrayLayerCount });
        t.msaaTextures[s] = i;
      }
...
Q.extension = { type: [l.WebGPUSystem], name: "renderTarget" };
class J {
  constructor() {
  ...
  _createGPUProgramData(e) {
    ...
    return this._gpuProgramData[e._layoutKey] = { bindGroups: r, pipeline: t.createPipelineLayout(s) }, this._gpuProgramData[e._layoutKey];
  }
  destroy() {
    this._gpu = null, this._gpuProgramData = null;
  }
}
J.extension = { type: [l.WebGPUSystem], name: "shader" };
const _ = {};
_.normal = { alpha: { srcFactor: "one", dstFactor: "one-minus-src-alpha", operation: "add" }, color: { srcFactor: "one", dstFactor: "one-minus-src-alpha", operation: "add" } };
_.add = { alpha: { srcFactor: "src-alpha", dstFactor: "one-minus-src-alpha", operation: "add" }, color: { srcFactor: "one", dstFactor: "one", operation: "add" } };
_.multiply = { alpha: { srcFactor: "one", dstFactor: "one-minus-src-alpha", operation: "add" }, color: { srcFactor: "dst", dstFactor: "one-minus-src-alpha", operation: "add" } };
_.screen = { alpha: { srcFactor: "one", dstFactor: "one-minus-src-alpha", operation: "add" }, color: { srcFactor: "one", dstFactor: "one-minus-src", operation: "add" } };
_.overlay = { alpha: { srcFactor: "one", dstFactor: "one-minus-src-alpha", operation: "add" }, color: { srcFactor: "one", dstFactor: "one-minus-src", operation: "add" } };
_.none = { alpha: { srcFactor: "one", dstFactor: "one-minus-src-alpha", operation: "add" }, color: { srcFactor: "zero", dstFactor: "zero", operation: "add" } };
_["normal-npm"] = { alpha: { srcFactor: "one", dstFactor: "one-minus-src-alpha", operation: "add" }, color: { srcFactor: "src-alpha", dstFactor: "one-minus-src-alpha", operation: "add" } };
_["add-npm"] = { alpha: { srcFactor: "one", dstFactor: "one", operation: "add" }, color: { srcFactor: "src-alpha", dstFactor: "one", operation: "add" } };
_["screen-npm"] = { alpha: { srcFactor: "one", dstFactor: "one-minus-src-alpha", operation: "add" }, color: { srcFactor: "src-alpha", dstFactor: "one-minus-src", operation: "add" } };
_.erase = { alpha: { srcFactor: "zero", dstFactor: "one-minus-src-alpha", operation: "add" }, color: { srcFactor: "zero", dstFactor: "one-minus-src", operation: "add" } };
_.min = { alpha: { srcFactor: "one", dstFactor: "one", operation: "min" }, color: { srcFactor: "one", dstFactor: "one", operation: "min" } };
_.max = { alpha: { srcFactor: "one", dstFactor: "one", operation: "max" }, color: { srcFactor: "one", dstFactor: "one", operation: "max" } };
class ee {
  constructor() {
    this.defaultState = new A(), this.defaultState.blend = true;
  }
  contextChange(e) {
    this.gpu = e;
  }
  getColorTargets(e, t) {
    const r = _[e.blendMode] || _.normal, s = [], i = { format: "bgra8unorm", writeMask: 0, blend: r };
...
ee.extension = { type: [l.WebGPUSystem], name: "state" };
const Ie = { type: "image", upload(a, e, t, r = 0) {
  const s = a.resource, i = (a.pixelWidth | 0) * (a.pixelHeight | 0), n = s.byteLength / i;
  t.device.queue.writeTexture({ texture: e, origin: { x: 0, y: 0, z: r } }, s, { offset: 0, rowsPerImage: a.pixelHeight, bytesPerRow: a.pixelWidth * n }, { width: a.pixelWidth, height: a.pixelHeight, depthOrArrayLayers: 1 });
} }, te = { "bc1-rgba-unorm": { blockBytes: 8, blockWidth: 4, blockHeight: 4 }, "bc2-rgba-unorm": { blockBytes: 16, blockWidth: 4, blockHeight: 4 }, "bc3-rgba-unorm": { blockBytes: 16, blockWidth: 4, blockHeight: 4 }, "bc7-rgba-unorm": { blockBytes: 16, blockWidth: 4, blockHeight: 4 }, "etc1-rgb-unorm": { blockBytes: 8, blockWidth: 4, blockHeight: 4 }, "etc2-rgba8unorm": { blockBytes: 16, blockWidth: 4, blockHeight: 4 }, "astc-4x4-unorm": { blockBytes: 16, blockWidth: 4, blockHeight: 4 } }, We = { blockBytes: 4, blockWidth: 1, blockHeight: 1 }, He = { type: "compressed", upload(a, e, t, r = 0) {
  ...
  for (let o = 0; o < a.resource.length; o++) {
    ...
    t.device.queue.writeTexture({ texture: e, mipLevel: o, origin: { x: 0, y: 0, z: r } }, u, { offset: 0, bytesPerRow: c }, { width: Math.ceil(s / n.blockWidth) * n.blockWidth, height: Math.ceil(i / n.blockHeight) * n.blockHeight, depthOrArrayLayers: 1 }), s = Math.max(s >> 1, 1), i = Math.max(i >> 1, 1);
  }
} }, E = ["right", "left", "top", "bottom", "front", "back"];
function Ve(a) {
  return { type: "cube", upload(e, t, r) {
    const s = e.faces;
...
const re = { type: "image", upload(a, e, t, r = 0) {
  const s = a.resource;
  if (!s) return;
  ...
  t.device.queue.copyExternalImageToTexture({ source: s }, { texture: e, origin: { x: 0, y: 0, z: r }, premultipliedAlpha: o }, { width: i, height: n });
} }, Ne = { type: "video", upload(a, e, t, r) {
  re.upload(a, e, t, r);
} };
class je {
  constructor(e) {
  ...
  generateMipmap(e) {
    ...
    if (!i) {
      const u = { size: { width: Math.ceil(e.width / 2), height: Math.ceil(e.height / 2), depthOrArrayLayers: s }, format: e.format, usage: GPUTextureUsage.TEXTURE_BINDING | GPUTextureUsage.COPY_SRC | GPUTextureUsage.RENDER_ATTACHMENT, mipLevelCount: e.mipLevelCount - 1 };
      r = this.device.createTexture(u);
    }
    const n = this.device.createCommandEncoder({}), o = t.getBindGroupLayout(0);
    for (let u = 0; u < s; ++u) {
      let c = e.createView({ baseMipLevel: 0, mipLevelCount: 1, dimension: "2d", baseArrayLayer: u, arrayLayerCount: 1 }), g = i ? 1 : 0;
      for (let h = 1; h < e.mipLevelCount; ++h) {
        const p = r.createView({ baseMipLevel: g++, mipLevelCount: 1, dimension: "2d", baseArrayLayer: u, arrayLayerCount: 1 }), d = n.beginRenderPass({ colorAttachments: [{ view: p, storeOp: "store", loadOp: "clear", clearValue: { r: 0, g: 0, b: 0, a: 0 } }] }), f = this.device.createBindGroup({ layout: o, entries: [{ binding: 0, resource: this.sampler }, { binding: 1, resource: c }] });
        d.setPipeline(t), d.setBindGroup(0, f), d.draw(3, 1, 0, 0), d.end(), c = p;
      }
    }
    if (!i) {
      const u = { width: Math.ceil(e.width / 2), height: Math.ceil(e.height / 2), depthOrArrayLayers: s };
      for (let c = 1; c < e.mipLevelCount; ++c) n.copyTextureToTexture({ texture: r, mipLevel: c - 1 }, { texture: e, mipLevel: c }, u), u.width = Math.ceil(u.width / 2), u.height = Math.ceil(u.height / 2);
    }
...
class se {
  constructor(e) {
    this._gpuSamplers = /* @__PURE__ */ Object.create(null), this._bindGroupHash = /* @__PURE__ */ Object.create(null), this._renderer = e, e.gc.addCollection(this, "_bindGroupHash", "hash"), this._managedTextures = new D({ renderer: e, type: "resource", onUnload: this.onSourceUnload.bind(this), name: "gpuTextureSource" });
    const t = { image: re, buffer: Ie, video: Ne, compressed: He };
    this._uploads = { ...t, cube: Ve(t) };
  }
  get managedTextures() {
  ...
  initSource(e) {
    var _a;
    return ((_a = e._gpuData[this._renderer.uid]) == null ? void 0 : _a.gpuTexture) || this._initSource(e);
  }
  _initSource(e) {
  ...
  getGpuSource(e) {
    var _a;
    return e._gcLastUsed = this._renderer.gc.now, ((_a = e._gpuData[this._renderer.uid]) == null ? void 0 : _a.gpuTexture) || this.initSource(e);
  }
  ...
  _createTextureBindGroup(e) {
    const t = e.source;
    return this._bindGroupHash[e.uid] = new k({ 0: t, 1: t.style, 2: new z({ uTextureMatrix: { type: "mat3x3<f32>", value: e.textureMatrix.mapCoord } }) }), this._bindGroupHash[e.uid];
  }
  getTextureView(e) {
    const t = e.source;
  ...
  generateCanvas(e) {
    ...
    return i.configure({ device: t.gpu.device, usage: GPUTextureUsage.COPY_DST | GPUTextureUsage.COPY_SRC, format: B.get().getNavigator().gpu.getPreferredCanvasFormat(), alphaMode: "premultiplied" }), r.copyTextureToTexture({ texture: t.texture.getGpuSource(e.source), origin: { x: 0, y: 0 } }, { texture: i.getCurrentTexture() }, { width: s.width, height: s.height }), t.gpu.device.queue.submit([r.finish()]), s;
  }
  getPixels(e) {
  ...
  destroy() {
    ...
    for (const e of Object.keys(this._bindGroupHash)) {
      const t = Number(e);
      (_a = this._bindGroupHash[t]) == null ? void 0 : _a.destroy();
    }
    this._renderer = null, this._gpu = null, this._mipmapGenerator = null, this._gpuSamplers = null, this._bindGroupHash = null;
  }
}
se.extension = { type: [l.WebGPUSystem], name: "texture" };
class ie {
  constructor() {
    this._maxTextures = 0;
  }
  contextChange(e) {
    const t = new z({ uTransformMatrix: { value: new F(), type: "mat3x3<f32>" }, uColor: { value: new Float32Array([1, 1, 1, 1]), type: "vec4<f32>" }, uRound: { value: 0, type: "f32" } });
    this._maxTextures = e.limits.maxBatchableTextures;
    const r = O({ name: "graphics", bits: [fe, ge(this._maxTextures), Se, I] });
    this.shader = new W({ gpuProgram: r, resources: { localUniforms: t } });
  }
  execute(e, t) {
    ...
    const g = i.globalUniforms.bindGroup;
...
ie.extension = { type: [l.WebGPUPipesAdaptor], name: "graphics" };
class ne {
  init() {
    const e = O({ name: "mesh", bits: [Te, Ce, I] });
    this._shader = new W({ gpuProgram: e, resources: { uTexture: M.EMPTY._source, uSampler: M.EMPTY._source.style, textureUniforms: { uTextureMatrix: { type: "mat3x3<f32>", value: new F() } } } });
  }
  execute(e, t) {
    const r = e.renderer;
    let s = t._shader;
...
ne.extension = { type: [l.WebGPUPipesAdaptor], name: "mesh" };
const qe = [...Ue, $, K, U, q, N, se, Q, J, ee, Z, j, Y, V], Ye = [...we, X], $e = [H, ne, ie], oe = [], ae = [], ue = [];
...
class rt extends me {
  constructor() {
    const e = { name: "webgpu", type: _e.WEBGPU, systems: oe, renderPipes: ae, renderPipeAdaptors: ue };
    super(e);
  }
}
export {
  rt as WebGPURenderer
};
